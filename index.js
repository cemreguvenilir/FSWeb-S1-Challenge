...
function fenomenEkle(feno, sirano, profil, takipci, gonderi, yeniPlatform) {
  let eklefeno = [...feno]
  const yeniKisi = {
    number: sirano,
    profile: profil,
    followers: takipci,
    posts: gonderi,
    platform: yeniPlatform

  }
  eklefeno.push(yeniKisi);
  return eklefeno;
}
...
function platformaGoreCokGonderiYapanFenomen(feno, platform1){
  let yeniDizi = [];
  for (let i=0; i<feno.length; i++) {
    if(feno[i].platform == platform1) {
      yeniDizi.push(feno[i]);
    }
  }
  const maxFeno = yeniDizi.sort(function (ilkFeno, ikinciFeno) {
